...
public class GLRasterizerState implements GraphicsResource
{
    ...
    @Override
    public void close() {
        if (cullModeSet)  GLRasterizer.popCullMode();
        if (fillModeSet)  GLRasterizer.popFillMode();
        if (frontFaceSet) GLRasterizer.popFrontFace();
        if (scissorSet)   GLRasterizer.popScissor();
        if (viewportSet)  GLRasterizer.popViewport();
    }
